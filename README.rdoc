...
* mysql (MySQL/Ruby MySQL API mudule) - http://www.tmtm.org/mysql/ruby/README.html
...
== Change Log
* *NEW* (v.0.2.2): Table.with_interval is introduced. This class methods call 'with_scope' protected method internally. Table#find_(all_)by_interval is now deprecated. Sorry for an inconstant API. However, this change enable combination queries using genomic intervals and any fields. 
* *NEW* (v.0.2.2): Bio::GenomicInterval#bin_all and Bio::GenomicInterval#bin return the bin index for the given interval.
* *NEW* (v.0.2.2): Supporting JRuby 1.6.3 or later. Appropiate Java heap size may have to be specified to invoke JRuby, especially when you use Bio::Ucsc::Reference. Try "jruby -J-Xmx3g your_script.rb" to keep 3G byte heap.
...
Table search using genomic intervals:
 ...
 Ucsc::Hg19::Snp131.with_interval(gi) do
   Ucsc::Hg19::Snp131.find(:all).each do |e|
 ...
 Ucsc::Hg19::Snp131.with_interval(gi) do
   p Ucsc::Hg19::Snp131.find(:all)
 end

 Ucsc::Hg19::Snp131.with_interval(gi, partial => false) do 
   p Ucsc::Hg19::Snp131.find(:all)
 end  

 p Ucsc::Hg19::Snp131.find_by_name("rs56289060")
...
find_by_*_and_* methods
 gi = GenomicInterval.parse("chr17:7,579,614-7,579,700" 
 Bio::Ucsc::Hg19::Snp132.with_interval(gi) do
   Bio::Ucsc::Hg19::Snp132.find_all_by_class_and_strand("in-del", "+")
 end
...
* Special field names such as 'id', 'attribute', 'valid', 'validate', 'class', 'method', 'methods', and 'type' are not supported by "find_(all_)by_[field name]" methods. Instead, use the find method with specifying :condition. For example, the "id" fielse can be search by "find(:all, :conditions=>["id = ?", 123])". ":all" can be replaced by ":first".
== Copyright
