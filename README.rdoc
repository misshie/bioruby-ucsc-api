...
== Change Log
* *NEW* (v.0.3.0): Table.with_interval is introduced. This class methods call 'with_scope' protected method internally. Table#find_(all_)by_interval is now deprecated. Sorry for an inconstant API. However, this change enable combination queries using genomic intervals and any fields. 
...
* Queries using genomic intervals are supported by .with_intervals class method of the table class. The method takes a block for queries. The method accepts a Bio::GenomicInterval object containing a genomic interval such as "chr1:1233-5678". If a table to query has the "bin"column, the bin index system is automatically used to speed-up the query.
