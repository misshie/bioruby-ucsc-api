...
* Special field names such as 'id', 'attribute', 'valid', 'validate', 'class', 'method', 'methods', and 'type' are not supported by "find_(all_)by_[field name]" methods. Instead, use the find method with specifying :confition. For example, the "id" fielse can be search by "find(:all, :conditions=>["id = ?", 123])". ":all" can be replaced by ":first".
== Copyright
