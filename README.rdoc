...
== Supported tables
* ...
* Table names starting with a number: Because Ruby class names cannot start with number, start the table class with "T" (T for Table) on the table name. Thus the "2micron_est" table is supprted by the "T2micron_est" class.
  
== Change Log
* *NEW* (v.0.2.1): New genome assemblies are supported: [chimp] PanTro3, [orangutan] PonAbe2, [rhesus] RheMac2 [rat] Rn4, [medaka] OryLat2, [sea squirt] Ci2, [yeast] SacCer2
