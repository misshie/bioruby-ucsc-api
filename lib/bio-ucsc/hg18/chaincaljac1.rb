...
Chr20 Chr21 Chr22 ChrX ChrY).each do |chr|
          klass = Class.new(DBConnection) do
            ...
            set_table_name "#{chr.downcase}_chainCalJac1"
            set_primary_key nil
          end
          self.const_set("#{chr}_ChainCalJac1", klass)
        end

        def self.find_by_interval(interval)
          chr_klass = self.const_get("#{interval.chrom.capitalize}_ChainCalJac1")
