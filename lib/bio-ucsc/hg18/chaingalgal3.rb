    ...
    module Hg18
      ...
      class ChainGalGal3
        ...
        Bio::Ucsc::Hg18::CHROMS.each do |chr|
          class_eval %!
            class #{chr[0..0].upcase + chr[1..-1]}_ChainGalGal3 < DBConnection
              self.table_name = "#{chr[0..0].downcase + chr[1..-1]}_chainGalGal3"
              set_primary_key nil
...
AND  (tEnd BETWEEN :zstart AND :zend))
        ...
        def self.find_by_interval(interval, opt = {:partial => true}); interval = Bio::Ucsc::Gi.wrap(interval)
          ...
          chr_klass = self.const_get("#{chrom}_ChainGalGal3")
