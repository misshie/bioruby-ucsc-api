    ...
    module Hg18
      ...
      class ChainOryLat2
        ...
        Bio::Ucsc::Hg18::CHROMS.each do |chr|
          class_eval %!
            class #{chr[0..0].upcase + chr[1..-1]}_ChainOryLat2 < DBConnection
              self.table_name = "#{chr[0..0].downcase + chr[1..-1]}_chainOryLat2"
              set_primary_key nil
...
AND  (tEnd BETWEEN :zstart AND :zend))
        ...
        def self.find_by_interval(interval, opt = {:partial => true})
          ...
          chr_klass = self.const_get("#{chrom}_ChainOryLat2")
