...
ChrX ChrY).each do |chr|
          class_eval %!
            class #{chr}_ChainAnoCar1Link < DBConnection
              set_table_name "#{chr.downcase}_chainAnoCar1Link"
              set_primary_key nil
...
AND  (tEnd BETWEEN :zstart AND :zend))
        ...
        def self.find_by_interval(interval, opt = {:partial => true})
          chr_klass = self.const_get("#{interval.chrom.capitalize}_ChainAnoCar1Link")
