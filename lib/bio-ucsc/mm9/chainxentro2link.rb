...
module Bio
  module Ucsc
    module Mm9
      ...
      class ChainXenTro2Link
        KLASS = "ChainXenTro2Link"
        KLASS_S = "chainXenTro2Link"

        %w(ChrM Chr1 Chr2 Chr3 Chr4 Chr5 Chr6 Chr7 Chr8 Chr9
Chr10 Chr11 Chr12 Chr13 Chr14 Chr15 Chr16 Chr17 Chr18 Chr19
ChrX ChrY).each do |chr|
          class_eval %!
            class #{chr}_#{KLASS} < DBConnection
              set_table_name "#{chr.downcase}_#{KLASS_S}"
              set_primary_key nil
...
AND  (tEnd BETWEEN :zstart AND :zend))
        ...
        def self.find_by_interval(interval, opt = {:partial => true})
          chr_klass = self.const_get("#{interval.chrom.capitalize}_#{KLASS}")
